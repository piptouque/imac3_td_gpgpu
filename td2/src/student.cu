...
// ==================================================
    ...
    )
    {
        ...
        HANDLE_ERROR(
                cudaMemcpy(
                        dev_inputImg, inputImg.data(), imgWidth * imgHeight * sizeof(uchar4), cudaMemcpyHostToDevice
                          ));
        HANDLE_ERROR(
                cudaMemcpy(dev_matConv, matConv.data(), matSize * matSize * sizeof(float), cudaMemcpyHostToDevice));
        ...
        // device -> host, dealloc
        HANDLE_ERROR(
                cudaMemcpy(output.data(), dev_output, imgWidth * imgHeight * sizeof(uchar4), cudaMemcpyDeviceToHost));
    ...
    )
    {
        ...
        HANDLE_ERROR(
                cudaMemcpyToSymbol(
                        cst_matConv, matConv.data(), matSize * matSize * sizeof(float), 0, cudaMemcpyHostToDevice
                                  ));
    ...
    )
    {
        ...
        assert(
                imgWidth * imgHeight < maxTexture1D &&
                "1-dimensional texture memory is not enough for the image, use 2D."
              );
        ...
        HANDLE_ERROR(
                cudaMemcpyToArray(
                        arr_inputImg, 0, 0, inputImg.data(), imgWidth * imgHeight * sizeof(uchar4),
    ...
    )
    {
        ...
        HANDLE_ERROR(
                cudaMallocPitch(
                        reinterpret_cast<void **>(&dev_output),
                        &pitch,
